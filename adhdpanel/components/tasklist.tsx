...
export default function TaskPanel() {
  ...
  useEffect(() => {
    const initializeRemoteStorage = async () => {
      try {
        const remoteStorageEnabled = await AsyncStorage.getItem(REMOTE_STORAGE_ENABLED_KEY);
        if (remoteStorageEnabled === 'true') {
          const userId = await AsyncStorage.getItem(REMOTE_STORAGE_USER_ID_KEY);
          const instanceId = await AsyncStorage.getItem(REMOTE_STORAGE_INSTANCE_ID_KEY);
          const serverAddress = await AsyncStorage.getItem(REMOTE_STORAGE_SERVER_ADDRESS_KEY);
          
          if (userId && instanceId && serverAddress) {
            ...
            // Test remote storage
            try {
              await rs.setItem('test_key', 'test_value');
              const testValue = await rs.getItem('test_key');
              if (testValue === 'test_value') {
                console.log('Remote storage is working correctly');
                setRemoteStorage(rs);
              } else {
                console.error('Remote storage test failed');
                setRemoteStorage(null);
              }
            } catch (error) {
              console.error('Error testing remote storage:', error);
              setRemoteStorage(null);
            }
          } else {
            console.log('Remote storage settings are incomplete');
            setRemoteStorage(null);
          }
        } else {
  ...
  const loadGithubData = async () => {
    try {
      const storedGithubUsername = remoteStorage 
        ? await remoteStorage.getItem('githubUsername')
        : await AsyncStorage.getItem('githubUsername');
  ...
  const loadAgixtData = async () => {
    try {
      const storedAgixtApiUri = remoteStorage
        ? await remoteStorage.getItem(AGIXT_API_URI_KEY)
        : await AsyncStorage.getItem(AGIXT_API_URI_KEY);
      const storedAgixtApiKey = remoteStorage
        ? await remoteStorage.getItem(AGIXT_API_KEY_KEY)
        : await AsyncStorage.getItem(AGIXT_API_KEY_KEY);
      if (storedAgixtApiUri && storedAgixtApiKey) {
        setAgixtApiUri(storedAgixtApiUri);
        setAgixtApiKey(storedAgixtApiKey);
      }
    } catch (error) {
      console.error('Error getting AGiXT data:', error);
    }
  };
  ...
  const loadTasks = async () => {
    try {
      let storedTasks;
      if (remoteStorage) {
        storedTasks = await remoteStorage.getItem("tasks");
        console.log('Tasks loaded from remote storage');
      } else {
  ...
  const saveTasks = async (updatedTasks) => {
    try {
      if (remoteStorage) {
        await remoteStorage.setItem("tasks", JSON.stringify(updatedTasks));
        console.log('Tasks saved to remote storage');
      } else {
...
const AGiXTModal = ({ visible, onClose, onAgentSelect, agixtApiUri, agixtApiKey, chains, remoteStorage }) => {
  ...
  useEffect(() => {
    const fetchAgentsAndChains = async () => {
      ...
      try {
        ...
        const storedAlwaysUseAgent = await (remoteStorage 
          ? remoteStorage.getItem(ALWAYS_USE_AGENT_KEY)
          : AsyncStorage.getItem(ALWAYS_USE_AGENT_KEY));
  ...
  }, [agixtApiUri, agixtApiKey, remoteStorage]);
  ...
  const handleAlwaysUseAgentChange = async () => {
    ...
    try {
      if (remoteStorage) {
        await remoteStorage.setItem(ALWAYS_USE_AGENT_KEY, JSON.stringify(newValue));
        console.log('Always use agent preference saved to remote storage');
      } else {
        await AsyncStorage.setItem(ALWAYS_USE_AGENT_KEY, JSON.stringify(newValue));
        console.log('Always use agent preference saved to local storage');
      }
    } catch (error) {
      console.error('Error saving always use agent preference:', error);
    }
  };
