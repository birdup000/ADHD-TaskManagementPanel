...
export default function TaskPanel() {
  ...
  const renderTaskItem = useCallback(({ item }) => (
    <TaskCard
      ...
      darkMode={darkMode}
    />
  ), [editTask, removeTask, onToggleComplete, showDependentTasks, tasks, setSelectedTaskForAGiXT, darkMode]);
        ...
        >
          <View style={[styles.modalOverlay, darkMode && styles.darkModeOverlay]}>
            <View style={[styles.modalContent, darkMode && styles.darkModeModalContent]}>
              ...
              <TextInput
                style={[styles.input, darkMode && styles.darkModeInput]}
                value={selectedTask ? selectedTask.text : newTaskText}
                onChangeText={(text) => 
                  selectedTask 
              ...
              />
              <CustomPicker
                selectedValue={selectedTask ? selectedTask.priority : newTaskPriority}
                onValueChange={(value) => 
                  selectedTask
              ...
              />
              <EnhancedDatePicker
                selected={selectedTask && selectedTask.dueDate ? new Date(selectedTask.dueDate) : null}
                onChange={(date) => {
                  if (selectedTask) {
                    setSelectedTask({...selectedTask, dueDate: date.toISOString()});
                  }
                }}
                darkMode={darkMode}
              />
...
const AlertModal = ({ visible, title, message, onClose, darkMode }) => {
  return (
    ...
    >
      <View style={[styles.modalOverlay, darkMode && styles.darkModeOverlay]}>
        <View style={[styles.modalContent, darkMode && styles.darkModeModalContent]}>
          <Text style={[styles.modalTitle, darkMode && styles.darkModeText]}>{title}</Text>
